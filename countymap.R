...
# color gradient
col_grad <- wes_palette("Zissou1", length(unique(countycase_map$CountyName)), type = "continuous")
...
countyplotlist[["rep"]] <- ggplot(countycase_map) + 
  ...
  ggtitle("Covid cases per 100,000 population by county", 
          subtitle = paste("Date:", format.Date(latest_date, "%B %d, %Y"))) +
